...
| `GET` | `/api/tasks` | 获取当前运行中的任务列表及状态 |
| `DELETE` | `/api/tasks/{tid}` | 取消正在运行的任务（如果正在下载/更新该 tid） |
...
- GET 返回所有 section 及 key-value（不含 `[server]` 中的 `password`）
...
WebSocket 也需 Basic Auth 认证（通过 URL query 参数 `?token=base64(admin:password)` 或首条消息认证）。
...
- 按钮：开始下载
- 下载启动后，显示实时进度区域：
  - 阶段指示器：下载 → 内容处理 → 资源下载 → 生成 Markdown
  - 页码进度条：当前页/总页
  - 楼层进度：当前楼/总楼
...
顶部导航栏，四个页面通过 tab 或链接切换。单页应用风格（各页面独立 HTML 文件，通过导航跳转）。
...
2. **server/server.go**: 新文件，HTTP 服务器、路由、中间件（Basic Auth）
3. **server/api.go**: 新文件，API handler 实现
4. **server/ws.go**: 新文件，WebSocket handler 及进度推送
5. **server/schedule.go**: 新文件，定时任务管理（CRUD + goroutine cron 定时器）
6. **server/frontend.go**: 新文件，静态文件 serve（go:embed）
7. **server/frontend/**: 新目录，存放前端 HTML/CSS/JS 文件
8. **nga/nga.go**: 修改 `Tiezi.Download()` 和相关方法，增加进度回调机制（`ProgressCallback`），供 Server 模式调用时传入回调以实时报告进度
9. **config/config.go**: 新增 `[server]` section 支持（`password`、`port` key）
...
- 同一 tid 不能同时运行两个下载/更新任务（返回 409 Conflict）
