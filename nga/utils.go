...
func downloadAssets(url string, fileName string) {
	...
	_, err := client.ImpersonateChrome().R().SetOutputFile(fileName).Get(url)
	if err != nil {
		log.Printf("下载资源失败: %s -> %s, 错误: %v\n", url, fileName, err)
	}
}
...
// 找不到时会直接返回 ""
func FindFolderNameByTid(tid int, authorId int) (string, error) {
	...
	if err == nil && fi.IsDir() {
		...
		return "", fmt.Errorf("检查文件夹 %s 失败: %v", folderName, err)
	}
	// 即不存在直接以 tid 命名的文件夹，接下来判断是否存在以 tid- 开头的文件夹
	if authorId > 0 {
		folderName = filepath.Join(".", fmt.Sprintf("%d(%d)-*", tid, authorId))
	} else {
		folderName = filepath.Join(".", fmt.Sprintf("%d-*", tid))
	}
	matches, err := filepath.Glob(folderName)
	if err != nil {
		return "", fmt.Errorf("匹配文件夹 %s 失败: %v", folderName, err)
	}
	if len(matches) == 1 {
		return matches[0], nil
	} else if len(matches) > 1 {
		return "", fmt.Errorf("有多个文件夹匹配: %s", folderName)
	}
	return "", nil
}