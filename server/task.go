...
func (tm *TaskManager) EnqueueDownload(tid int, authorId int) (*TaskStatus, error) {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
func (tm *TaskManager) EnqueueUpdate(tid int) (*TaskStatus, error) {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
// Helper: 将 TaskStatus 转为 JSON
func taskStatusToJSON(task *TaskStatus) []byte {
	data, _ := json.Marshal(task)
	return data
}

// Helper: cast wrapper for convenience
